...
func main() {
	...
	fmt.Println("Arreglo con un elemento agregado:", numeros)

	// Eliminar un elemento de un arreglo
	numeros = append(numeros[:2], numeros[3:]...)
	fmt.Println("Arreglo con un elemento eliminado:", numeros)

	// Obtener un subconjunto de un arreglo
	subconjunto := numeros[1:3]
	fmt.Println("Subconjunto del arreglo:", subconjunto)
...
// # bash
// numeros=(5 1 3 2 4)
// mapfile -t numeros_ordenados < <(echo "${numeros[@]}" | sort -n)
...
// mapfile -t numeros_ordenados_desc < <(echo "${numeros[@]}" | sort -nr)
