...
# El comando "mapfile" se utiliza para leer la salida de un comando y almacenar cada línea en un elemento de un array. En este caso, se usa para leer la salida ordenada del comando "sort" y almacenar cada número ordenado en un elemento de "numeros_ordenados". Finalmente, se imprime el array ordenado.
...
echo "${numeros_ordenados_desc[@]}"

# El comando "mapfile" se utiliza de la misma manera que en el script anterior, pero en este caso se utiliza el comando "sort -nr" para ordenar los números en orden descendente en lugar de en orden ascendente. La salida ordenada se almacena en el array "numeros_ordenados_desc" y luego se imprime el array ordenado.
